...
The current stack is Android/iOS-only KMP with Gradle 9.6.1, AGP 9.3.1, Kotlin 2.4.10, and JDK 17 bytecode. Android is the affected distributable platform: its required Firebase secrets exist, while iOS signing and Firebase App Distribution secrets do not. The first remote verification run also showed that the v4 official setup/cache/artifact actions target deprecated Node 20 and are being forced onto Node 24 by GitHub-hosted runners.
...
- Use supported Node 24-based official action majors without hosted-runner deprecation annotations.
...
### Use current Node 24-based official action majors

All workflows will use `actions/checkout@v6`, `actions/setup-java@v5`, `actions/cache@v5`, and `actions/upload-artifact@v6`; Android delivery will use `actions/setup-node@v6` with package-manager auto-caching disabled because npm is used only to install the pinned Firebase CLI. These majors are the official Node 24 migrations and are supported by GitHub-hosted runners.

The alternative was to retain v4 actions and accept GitHub's forced runtime override. That preserves a deprecation warning and leaves execution dependent on compatibility behavior outside the workflow contract.
...
- [Node 24 action majors require runner 2.327.1 or newer] → All affected jobs use GitHub-hosted current runners, which satisfy the requirement; no self-hosted runner compatibility is claimed.
