...
- [ ] 6.1 Implement scrypt in `commonMain` over the cryptography library's PBKDF2-HMAC-SHA256, per Decision 14, since no Kotlin Multiplatform dependency provides a memory-hard derivation across Android and Apple. Verify: `commonTest` reproduces the RFC 7914 published test vectors.
- [ ] 6.2 Implement PIN-derived key material composed with the device-held secret, using the scrypt derivation from 6.1 before composition, and store the PIN-wrapped copy locally only. Verify: `commonTest` asserts unlock is derivation-based rather than comparison-based, and that the PIN wrap plus correct PIN yields no key under a different device secret.
- [ ] 6.3 Implement the failed-attempt counter in secure storage with delay from the fifth consecutive failure starting at 30 seconds and doubling to a 5-minute cap, destruction of local key material at the tenth consecutive failure, reset on success, and persistence across reinstall. Verify: platform tests cover the delay schedule, destruction at ten, reset, reinstall persistence, and that phrase restoration still works after destruction.
- [ ] 6.4 Implement biometric unlock as a shortcut requiring a PIN, with a biometric-invalidating key access policy and PIN fallback. Verify: platform tests cover enabling without a PIN being refused, successful biometric unlock, cancellation falling back to PIN, and a simulated enrollment change leaving the PIN path working.
- [ ] 6.5 Implement in-memory-only data key handling with clearing on background past the auto-lock timeout, and unlock required before finance data is displayed. Verify: tests cover backgrounding past and within the timeout, and assert no unwrapped key is persisted outside process memory while unlocked.
...
- [ ] 6.7 Confirm no unlock screen is presented to a user who has not completed encryption setup. Verify: a local-only launch test asserts direct entry to the main experience.
- [ ] 6.8 Build recovery-phrase re-display in security settings, requiring the current PIN at the moment of the request even when the app is already unlocked. Verify: UI tests assert the phrase is shown only after correct PIN entry, that dismissing or failing the prompt shows nothing, and that an already-unlocked session is still prompted.
