...
class RecoveryPhraseKeyWrapper(private val random: CryptographyRandom = CryptographyRandom) {
    ...
    private companion object {
        /**
         * The HKDF domain separator for this wrap type, deliberately different from the device
         * wrap's.
         *
         * Both wraps end in an AES-256-GCM seal of the same 32 bytes, so the separator is what keeps
         * their wrapping keys unrelated and stops a copy sealed for one context from being opened by
         * a key derived for the other. It matters most here because this is the copy the backend
         * holds: it must stay tied to the phrase and to nothing else the device can produce. The
         * trailing version lets a later build change what is derived without silently opening copies
