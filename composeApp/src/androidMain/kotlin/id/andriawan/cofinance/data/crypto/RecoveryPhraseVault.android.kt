 ...
 * depend on Keystore entries that a reinstall destroys.
 */
actual fun createRecoveryPhraseVault(): RecoveryPhraseVault = SealedRecoveryPhraseVault(
    ...
    writeSealed = { value -> preferences().edit().putString(PHRASE_KEY, value).apply() },
    clearSealed = { preferences().edit().remove(PHRASE_KEY).apply() }
)
